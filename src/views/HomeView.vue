...
import RollBox from "@/components/RollBox.vue";
...
function start() {
  ...
  if (realRolls[0] == realRolls[1] && realRolls[1] == realRolls[2]) {
    //if win
...
function handleHover() {
  // move
  if (probability(0.5)) {
    const dir = randomChoose([
      "top",
      "left",
      "right",
      "bottom",
    ]) as keyof typeof pos.value;
    pos.value[dir] += 300;
  }
  //diable
  if (probability(0.3)) {
...
<template>
  <main>
    ...
    <table>
      ...
      <tr>
        <td>
          <RollBox :choice="rolls[0]" />
        </td>
        <td>
          <RollBox :choice="rolls[1]" />
        </td>
        <td>
          <RollBox :choice="rolls[2]" />
        </td>
      </tr>
    </table>
    <!-- cashout btn -->
